...
<div class="container-fluid">
                ...
                <div class="card-body bg-light rounded">
                    <form method="GET" class="row g-3 align-items-end">
                        ...
                        <div class="col-md-3">
                            ...
                            <a href="{{ route('admin.reports.users') }}" class="btn btn-outline-secondary btn-sm px-4">
    ...
    <div class="row mb-4">
        ...
        @foreach(['active', 'inactive', 'pending', 'banned'] as $statusName)
            <div class="col-xl-2 col-md-4 mb-3">
                <div class="card border-start border-{{ $statusColors[$statusName] ?? 'info' }} border-4 shadow-sm h-100 py-2">
                    <div class="card-body py-2">
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                <div class="text-xs font-weight-bold text-{{ $statusColors[$statusName] ?? 'info' }} text-uppercase mb-1">
                                    {{ ucfirst($statusName) }}
                                </div>
                                <div class="h5 mb-0 font-weight-bold text-gray-800">
                                    {{ number_format($userStats['users_by_status'][$statusName] ?? 0) }}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body py-2">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            ...
                            <div class="h5 mb-0 font-weight-bold text-gray-800">{{ number_format($userStats['filtered_users']) }}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-primary">{{ $userStats['users_list']->total() }} Users Found</span>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle">
                            ...
                            <tbody>
                                @forelse($userStats['users_list'] as $user)
                                    <tr>
                                        <td>{{ ($userStats['users_list']->currentPage() - 1) * $userStats['users_list']->perPage() + $loop->iteration }}</td>
                                        <td>
                                        ...
                                        <td>
                                            <span class="badge bg-{{ $user->status == 'active' ? 'success' : ($user->status == 'inactive' ? 'warning' : ($user->status == 'banned' ? 'danger' : 'secondary')) }}">
                                                {{ ucfirst($user->status) }}
                                            </span>
                                        </td>
                                        <td>
