...
<div class="card">
    ...
    <div class="card-body">
            ...
            <ol class="breadcrumb" id="breadcrumbList">
                ...
                @foreach($breadcrumbs as $crumb)
                    <li class="breadcrumb-item {{ $loop->last ? 'active' : '' }}">
                        @if(!$loop->last)
                            <a href="#" onclick="navigateTo('{{ $crumb['path'] }}')">
                                {{ $crumb['name'] }}
                            </a>
                        @else
                            {{ $crumb['name'] }}
        ...
        <input type="hidden" id="currentPathInput" value="{{ $path }}">
                ...
                <table class="table table-hover align-middle" id="fileTable">
                    ...
                    <tbody id="fileListBody">
                        ...
                        @foreach($directories as $directory)
                            <tr class="file-item" data-name="{{ $directory['name'] }}" data-type="folder">
                                <td>
                                    <i class="fas fa-folder text-warning me-2 fa-lg"></i>
                                    <a href="#" onclick="navigateTo('{{ $directory['path'] }}')"
                                ...
                                <td>-</td>
                                <td class="small text-muted">{{ $directory['modified'] }}</td>
                                <td class="text-end">
                                    <div class="btn-group btn-group-sm">
                                        <button class="btn btn-outline-danger" 
                                                onclick="deleteItem('{{ $directory['name'] }}', true)">
                        ...
                        @foreach($files as $file)
                            <tr class="file-item" data-name="{{ $file['name'] }}" data-type="file">
                                ...
                                <td><span class="badge bg-secondary">{{ strtoupper($file['type']) }}</span></td>
                                ...
                                <td class="text-end">
                                    <div class="btn-group btn-group-sm">
                                        ...
                                        @endif
                                        <a href="{{ route('admin.file-manager.download', $file['name']) }}?path={{ $path }}"
                                           class="btn btn-outline-primary">
                                            <i class="fas fa-download"></i>
                                        </a>
                                        <button class="btn btn-outline-secondary" 
                                                onclick="renameItem('{{ $file['name'] }}')">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button class="btn btn-outline-danger" 
                                                onclick="deleteItem('{{ $file['name'] }}', false)">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </div>
                                </td>
...
@push('scripts')
<script>
    let currentPath = @json($path);
    let currentFiles = @json($files);
    let currentDirectories = @json($directories);
    let viewMode = 'list';

    // Routes
    const routes = {
        index: @json(route('admin.file-manager.index')),
        upload: @json(route('admin.file-manager.upload')),
        createDirectory: @json(route('admin.file-manager.create-directory')),
        rename: @json(route('admin.file-manager.rename')),
        destroy: @json(route('admin.file-manager.destroy', '')),
        download: @json(route('admin.file-manager.download', ':file'))
    };
